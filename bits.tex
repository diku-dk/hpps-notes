...
This is also the form they take in programming languages such as C.
...
We can conser an unsigned number as consisting of an arbitrary number
...
A floating-point number format is characterised by four integers

\begin{itemize}
\item a \emph{radix}, which we will assume to be 2;
\item a \emph{precision} $p \geq 2$, roughly corresponding to the
  number of ``significant bits'' of the representation;
...
\end{equation}
To address this, we slightly refine the representation such that a
number $x$ is expressed as
\begin{equation}
  x = -1^{s} \cdot{} m \cdot{} 2^{e-p+1}
\end{equation}
where
...
\item $m=|M|\cdot{}2^{1-p}$ is called the \emph{normal significand}.
  Note that $0 \leq m < \beta$---intuitively, we can see $m$ as a
  number of the form $1.\ldots_{2}$, where $m$ is the bits to the
  right of the binal point.
\end{itemize}

The \emph{encoding} of a floating-point number as a bit vector
consists of three fields, $S$, $E$ and $T$.

\begin{definition}[Fields of a floating-point number]
  \[
...
\subsubsection{Normalised floating-point numbers}

We say that a floating-point number is \emph{normalised} when
$\textrm{Exp} \neq \bitvector{0\cdots 0}$ and
$\textrm{Exp} \neq \bitvector{1\cdots 1}$.

\begin{definition}[Decoding normal floating-point number]
  \[
    NormFP(\bitvector{S~E~T} = (-1)^{S} \cdot m \cdot 2^{e-p+1}
  \]
  where
  \[
    \begin{array}{ccr}
      b &=& 2^{W_{E}-1}-1 \\
      s &=& \mathrm{Bits2N}(\langle{S}) \\
      m &=& 1+\mathrm{Bits2N}(T) \times{} 2^{-m} \\
      e &=& \mathrm{Bits2N}(E) - b
    \end{array}
  \]
\end{definition}

Note that since $M\neq 0$, normalised floating-point
